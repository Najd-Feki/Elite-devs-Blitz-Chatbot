...
export default function ReclamationAdmin() {
  ...
  function handelDelete(id) {
    axios.delete('http://localhost:5000/reclamation/delete/' + id).then(() => {
      setReclamation(
        reclamations.filter((reclamation) => reclamation._id != id)
      );
    });
  }
        ...
        <table className='table'>
          ...
          <tbody>
            {reclamations.map((reclamation, index) => (
              <tr>
                <td>
                  <Link to={'/profile/' + reclamation._id}>
                    {' '}
                    {reclamation.userId}
                  </Link>
                </td>
                ...
                <td>
                  <button
                    ...
                    onClick={() => handelDelete(reclamation._id)}
                  >
                    Delete
                  </button>
                </td>
